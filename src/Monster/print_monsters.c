...
// SYSTEME D'AFFICHAGE VERTICAL
//  printf("/n");
//  for(int g=0;g<cnt;g++){
//      printf("%*sHealth: %d %s ", (int)(diff + (strlen(monster_art[0]) + 1)), "", tabn[g].Health,H);
//  }
//  printf("/n");
//  for(int g=0;g<cnt;g++){
//      printf(" %*sAttack: %d(Min)-%d(Max) ", (int)((strlen(monster_art[0]) + 1)), "",tabn[g].Attackmin, tabn[g].Attackmax);
//  }
//  printf("/n");

// for(int g=0;g<cnt;g++){
//     printf("%*sProtection: %d ", (int)(diff + (strlen(monster_art[0]) + 1)), "", tabn[g].Defense);
// }
//   for (int i = 0; i < count; i++) {
//      if (enemies[i]->health > 0) {
//          //on affiche que les monstres qui sont en vie
//          for (int line = 0; line < ASCII_LINES_M; line++) {
//             if(line==ASCII_LINES_M-1){
//                 printf("%*s%s", (int)(diff + i * (strlen(monster_art[0]) + 1)), "", monster_art[line]);
//             }else{
